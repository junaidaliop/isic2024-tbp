...
| **Stack — rank-avg[GBDT, nano@224]** | **[0.17430]{.metric-chip}** |

The stack reconstruction `rank-avg[gbdt, r224]` recomputes to **0.17430**, matching the canonical
...
| **Stack `gbdt+nano@224`** | **0.17430** | 15.84 | 2.46 | 60.9 | **best accuracy** on the whole study |
...
- The image expert only earns its place **inside the stack**: `+0.0054` pAUC for `+60 ms`. Whether
...
the visual confirmation that the +0.0054 lift is concentrated where it is scored, not smeared
...
would be untrustworthy. The strongest evidence the +0.0054 is real and not a fluke: the **stack
has the lowest std (0.0068) and wins or ties on *every* fold**. It is both the most accurate and
...
Our headline is **OOF CV 0.1743 (stack) / 0.1689 (tabular)**; a fair private-test projection is
